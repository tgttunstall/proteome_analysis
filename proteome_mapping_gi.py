...
Example call:
    ./label_clusters.py --fasta_dir pig/ --tsv_file <(uniq results_pig/Specie_protein_cluster.tsv) --out_file results_pig/Labelled_Specie_protein_cluster.tsv --prefix proteome_
"""
...
def check_args(DESCRIPTION):
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Optionally prefix for filenames e.g. 'proteome_', which will be removed",
    )
    ...
    try:
        ...
        eprint(
            f"    => ERROR: Cannot open file '{args.out_file}' for writing".format(
                filename
            )
        )
...
def create_proteome_protein_map(fasta_files, read_method="full"):
    """
    ...
    for file in fasta_files:
        ...
        if read_method == "chunks":  # read file in chunks
            for line in read_file_in_chunks(file):
                if line.startswith(">"):  # fasta header
                    _process_protein_id(line)
        elif read_method == "lines":  # read line by line
            with open(file, "r") as f:
                for line in f:
                    if line.startswith(">"):
                        _process_protein_id(line)
        else:  #'full' read whole file in memory
            with open(file, "r") as f:
...
def label_proteins(input_fh, output_file, proteome_protein_map):
    """
    ...
    with open(output_file, "w") as outfile:
        ...
        for line in input_fh:
            ...
            if cluster_id != prev_cluster:
                ...
                cluster_counter += 1
                outfile.write(
 f"{cluster_counter}\t{protein_id}\t{proteome_protein_map.get(protein_id, '')}\t*\n"
                )
            else:
                outfile.write(
 f"{cluster_counter}\t{protein_id}\t{proteome_protein_map.get(protein_id, '')}\t\n"
...
if __name__ == "__main__":
    ...
    proteome_protein_map = create_proteome_protein_map(fasta_files, read_method="full")
    eprint(
    ...
    # ===============
    start_secs = time.time()
    clusters_count = label_proteins(args.tsv_file, args.out_file, proteome_protein_map)
    timestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
